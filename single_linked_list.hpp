...
public:
    ...
    void push_back(const Type& value) {
        if(size_ == 0){
            ...
            std::cout << "end_.value before " << end_.value << std::endl;
            //std::cout << "end_.next_node.value " << end_.next_node->value << std::endl;
            end_.next_node = new Node(value, nullptr);  // обновляем указатель на последний
            end_ = *(end_.next_node);
            std::cout << "end_.value after " << end_.value << std::endl;
